...
const EachItem = ({ product, setProductItem }) => {
  ...
  //   queryKey: [sellerMail],
  //   queryFn: async () => {
  //     const res = await fetch(`https://used-laptop-server-iota.vercel.app/users/${sellerMail}`);
  ...
  // useEffect(() => {
  //   fetch(`https://used-laptop-server-iota.vercel.app/users/${sellerMail}`)
  //     .then((res) => res.json())
  //     .then((data) => {
  ...
  const handleReportProduct = (product) => {
    ...
    fetch(
      `https://used-laptop-server-iota.vercel.app/report/${product._id}`,
      {
        method: "PATCH",
        headers: {
